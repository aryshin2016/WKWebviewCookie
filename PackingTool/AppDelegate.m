...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if ([UIDevice currentDevice].systemVersion.doubleValue >= 8.0) {
        ...
        SQBaseWkViewController *view = [[SQBaseWkViewController alloc]init];
        view.httpString = [YSKeyManager shared].gameUrlString;
        self.window.rootViewController = view;
    }else{
        
        SQBaseWebViewController *view = [[SQBaseWebViewController alloc]init];
        view.urlString = [YSKeyManager shared].gameUrlString;
        self.window.rootViewController = view;
    }
...
-(void)setUserAgent{
    ...
    SQLog(@"old agent :%@", oldAgent);
    NSString *newAgent = @"IOSBox";
    SQLog(@"new agent :%@", newAgent);
